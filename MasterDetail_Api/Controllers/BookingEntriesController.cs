...
public class BookingEntriesController : ControllerBase
{
    ...
    {
        return await _context.Clients.Include(x => x.bookingEntries).ThenInclude(b => b.Spot).OrderByDescending(c=>c.ClientId).ToListAsync();
    }
        ...
        try
        {
            ...
            if (clientDTO.PictureFile != null)
            {
                ...
                var fileName = Guid.NewGuid().ToString() + 
                    Path.GetExtension(clientDTO.PictureFile.FileName);
    ...
    [Route("Deleteb/{id}")]
    [HttpDelete]
    public async Task<ActionResult<BookingEntry>> DeleteBooking(int id)
    {       
        BookingEntry bookingEntry = await _context.BookingEntries.FindAsync(id);
        
        if (bookingEntry == null)
        {
            return NotFound();
        }    
        _context.BookingEntries.Remove(bookingEntry);
     
        await _context.SaveChangesAsync();
       
        return Ok(bookingEntry);
    }
